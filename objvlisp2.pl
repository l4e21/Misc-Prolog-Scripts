...
% Method Resolution
% Bootstrapped version exists to search on class then upwards on object with no commits except for if at least one method is found
% But first try to send to the metaclass, recursion avoided via check on whether metaclass is our bootstrapped version
